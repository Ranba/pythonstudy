...
def main():
    ...
    # 主界面布局
    tk.Label(root, text="输入号码：").grid(row=0, column=0, padx=5, pady=5)
    number_input = tk.Text(root, height=10, width=40)
    number_input.grid(row=0, column=1, columnspan=2, padx=5, pady=5)

    tk.Button(root, text="查询", command=process_numbers).grid(row=1, column=1, pady=5)
    tk.Button(root, text="处理文件", command=process_file_dialog).grid(row=1, column=2, pady=5)

    tree = ttk.Treeview(root, columns=("号码", "区号", "省份", "城市", "运营商"), show="headings")
    tree.grid(row=2, column=0, columnspan=3, padx=5, pady=5)
